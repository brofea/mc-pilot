<div align="center">
    <img width="200" alt="Minecraft Pilot icon" src="web/src/assets/minecraft-pilot-icon.png" />
...
3. **游戏日志分析**：只读监听本地 `latest.log`，识别本地玩家和简中/英文死亡消息，每次有效死亡最多调用一次 OpenAI 兼容 API，并通过 WebSocket 推送一条短建议。
4. **轻量 Agent**：支持自然语言、通用复杂问题、MCP 风格 Function Calling、12 轮短期记忆、工具白名单、调用轮次限制与 token 预算。
5. **开发者后台**：展示系统、游戏、配方、RAG、LLM 和脱敏配置状态；仅允许本机访问。
6. **Vue 工作台**：Vue 3 + TypeScript + Vite 构建的响应式界面，统一承载对话、知识库、配方浏览、游戏连接与控制台导航。
...
- Agent 会直接认真回答安全的通用复杂问题；Minecraft 的版本相关事实和配方问题才按需调用 Wiki/配方工具。
- 用户消息、历史消息与工具返回均视为不可信数据；工具结果仅作为证据，不能修改身份、语气、工具权限或系统规则。
- 对话不会提供内部提示、模型/服务配置、部署细节、日志路径或凭据；试图覆盖规则或改变人格/语气的请求会得到稳定的边界回复。
- 单条消息最多 **12,000** 个字符，超限返回 HTTP `413`，不会调用模型或消耗 Token；每日共享预算为 **750,000 Token**。
- `/api/chat` 与 `/api/chat/stream` 共享每客户端 IP **20 次/分钟** 的内存级滑动窗口，超限返回 HTTP `429` 与 `Retry-After`。
- 离线评测语料位于 `tests/fixtures/agent_eval_cases.json`，覆盖复杂回答、工具选择、提示词注入、人格篡改与敏感信息索取；新增真实失败样例时应先加入该语料再修复。
...
当日志存在、识别到本地玩家且 OpenAI 兼容 API 已配置时，解析死亡消息并推送建议。
...
.
├── src/mc_pilot/
│   ├── agent/          # OpenAI 兼容 API 客户端、状态机、记忆与工具契约
...
[Minecraft Wiki 中文](https://zh.minecraft.wiki/api.php) 提供的 MediaWiki API

[FreeFrontend](https://freefrontend.com/css-liquid-glass/) 提供的 Liquid Glass CSS 效果
